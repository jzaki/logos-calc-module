...
CalcModulePlugin::CalcModulePlugin(QObject *parent) : QObject(parent) {
...
void CalcModulePlugin::initLogos(LogosAPI *api) {
  // IMPORTANT: Use the global `logosAPI` variable from liblogos, NOT a class
  // member. `logosAPI` is defined in the Logos SDK headers and is used by the
  // API internally. Storing the pointer in a local `m_logosAPI` member will NOT
  // work.
  logosAPI = api;
